...
import { getWeekIndexFromDate, isFixoWeekFromDate, getWeekIndex, isFixoWeek } from './week'
...
describe('API por Date (server) x API por string (client) — mesmo módulo', () => {
  // O #43 nasceu de client e server discordarem: a UI dizia "não pega nesta semana" e a
  // cobrança contava a semana. Antes eram dois arquivos duplicados; agora são duas APIs do
  // mesmo módulo — este teste continua sendo o que impede a divergência de voltar.
